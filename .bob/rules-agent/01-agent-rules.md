...
- All new credentials must also appear in `.env.example` with comments.
...
- All tests must pass (1 skip is intentional — live Groq smoke test).
...
1. ✅ `python -m pytest tests/ -v` — all passed, 1 skipped
2. ✅ No `.env` file staged (`git status` check)
3. ✅ Co-author line in commit message
